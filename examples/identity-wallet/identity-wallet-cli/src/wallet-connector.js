...
/**
 * Connects to the Midnight Lace Wallet and retrieves necessary information
 * For use with a browser environment that has access to window.midnight
 * 
 * @returns {Promise<Object>} Wallet information including address and coin public key
 */
export async function connectToWallet() {
  try {
    // Check if Midnight Lace Wallet is available
    if (!window.midnight) {
      console.error("Midnight Lace Wallet not found. Please ensure it's installed and enabled.");
      return null;
    }

    console.log("Midnight Lace Wallet detected. Attempting to connect...");
    
    // Access the mnLace interface which should be available on the wallet
    const mnLace = window.midnight.mnLace;
    if (!mnLace) {
      console.error("Midnight Lace mnLace interface not found.");
      return null;
    }

    // Display available methods for debugging
    console.log("Available wallet methods:", Object.keys(mnLace));

    // Request wallet connection/enable
    let accounts;
    if (typeof mnLace.enable === 'function') {
      ...
      if (typeof mnLace.getAccounts === 'function') {
        accounts = await mnLace.getAccounts();
      } else if (typeof mnLace.getUsedAddresses === 'function') {
        accounts = await mnLace.getUsedAddresses();
      } else if (typeof mnLace.getRewardAddresses === 'function') {
        accounts = await mnLace.getRewardAddresses();
      }
    } else if (typeof mnLace.request === 'function') {
      // Alternative method using request API
      accounts = await mnLace.request({ method: "wallet_requestAccounts" });
    }
    ...
    // Try different methods to get the coin public key
    let coinPublicKey;
    ...
    // Method 2: Try using request API
      ...
      try {
        coinPublicKey = await mnLace.request({ method: "wallet_getPublicKey" });
      } catch (error) {
        console.log("Could not get public key via wallet_getPublicKey:", error);
        
        // Try alternative method
        try {
          const walletInfo = await mnLace.request({ method: "wallet_getInfo" });
          if (walletInfo && walletInfo.publicKey) {
            coinPublicKey = walletInfo.publicKey;
          }
        } catch (innerError) {
          console.log("Could not get wallet info:", innerError);
        }
      }
    }

    // If we still don't have a coinPublicKey, check other properties
    if (!coinPublicKey && mnLace.publicKey) {
      coinPublicKey = mnLace.publicKey;
    }
    
    // Get network ID to verify we're on testnet
    let networkId;
    if (typeof mnLace.getNetworkId === 'function') {
      networkId = await mnLace.getNetworkId();
    } else if (typeof mnLace.request === 'function') {
      try {
        networkId = await mnLace.request({ method: "wallet_getNetworkId" });
      } catch (error) {
 ...
 */
export function createWalletConnectorHtml() {
  return `
<!DOCTYPE html>
<html>
<head>

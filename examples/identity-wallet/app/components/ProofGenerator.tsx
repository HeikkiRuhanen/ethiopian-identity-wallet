...
export default function ProofGenerator({ contractAddress }: { contractAddress?: string }) {
  ...
  useEffect(() => {
    ...
    setIsContractDeployed(!!contractAddress);
    if (contractAddress) {
  ...
  const createSoulBoundToken = async () => {
    if (!result || !result.proof) {
      setError('No proof available. Generate a proof first.');
      return;
    }
    ...
    try {
      ...
      // Note: Using mock Soul Bound Token implementation as the wallet doesn't support this yet
      console.log("Using mock Soul Bound Token implementation - wallet doesn't support this functionality yet");
      
      // Use the contract address if provided
      const effectiveContractAddress = contractAddress || 'default-contract-address';

      const submission = await LaceWalletService.createSoulBoundToken(
        tokenId,
        result.proof,
        credentialData,
        effectiveContractAddress // Pass the contract address
      );
